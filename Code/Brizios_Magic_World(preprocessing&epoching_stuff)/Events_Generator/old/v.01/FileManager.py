...
class FileManager:
    ...
    def loadData (self,  path, f1, f2):
        ...
        if (self.__isCSV(f1) and self.__isCSV(f2)) or (self.__isJSON(f1) and self.__isJSON(f2)):
            ...
            for f in files:
                if self.__isCSV(f):
                    self.data_csv = pd.read_csv(path + "/" + f)
                    self.file_name = path + "/" + f[: f.find('.')]
                elif self.__isJSON(f):
                    try:
                        fp = open(path + "/" + f)
                        self.data_json = json.load(fp)
                    except IOError:
                        print("An error has occurred while opening the file '{}'".format(f))
                        result = -1
    ...
    def generateEventsFile(self, round_times, answer_times):
        ...
        fp.write("Latency\tType\tPosition\n")
        
        for r in round_times:
            line = "" + str(r) + "\tnew-round\tround"
            fp.write(line + "\n")
        ...
        for a in answer_times:
            ...
            line = "" + str(a[3]) + "\tstart-answer\tanswer-window"
            fp.write(line + "\n")
            
            line = "" + str(a[4]) + "\tanswer-click\tanswer-click"
            fp.write(line + "\n")
            
            line = "" + str(a[5]) + "\tend-answer\tanswer-window"
            fp.write(line + "\n")
